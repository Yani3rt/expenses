...
- Choosing `All categories` closes the dropdown immediately.
...
The dropdown keeps the complete category catalog in a stable order, but availability follows the active search, period, and month filters. The availability query intentionally excludes current category selections so users can broaden a multiselect instead of being trapped within the first selection.

An unselected category with zero matching expenses remains visible but disabled and muted. A selected category remains enabled even if another filter change leaves it with zero matching expenses, ensuring it can always be removed. `All categories` is always enabled.
...
- Category availability counts use the same search/date predicates as the operation query, excluding only the category predicate.
