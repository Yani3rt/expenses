...
export function getTransactionDetailData({ id, q = "", period = "all", month = "all", category = "all" } = {}) {
  return withDatabase((db) => {
    const params = { id };
    const where = [];
    const cleanQuery = String(q || "").trim();
    ...
    const row = get(db, `
      WITH filtered AS (
        ...
        SELECT filtered.*,
               ROW_NUMBER() OVER (ORDER BY amount DESC, date DESC, id DESC) AS rank,
               COUNT(*) OVER () AS resultCount,
               SUM(amount) OVER () AS filteredTotal,
               AVG(amount) OVER () AS filteredAverage,
               ROW_NUMBER() OVER (
                 PARTITION BY categorySlug
                 ORDER BY amount DESC, date DESC, id DESC
               ) AS categoryRank,
               SUM(amount) OVER (PARTITION BY categorySlug) AS categoryTotal,
               AVG(amount) OVER (PARTITION BY categorySlug) AS categoryAverage
        FROM filtered
      )
      SELECT *,
             ROUND(amount * 100.0 / NULLIF(filteredTotal, 0), 2) AS spendSharePercent,
             ROUND(amount - filteredAverage, 2) AS differenceFromAverage,
             ROUND(filteredAverage, 2) AS roundedFilteredAverage,
             ROUND(categoryAverage, 2) AS roundedCategoryAverage,
             ROUND(amount * 100.0 / NULLIF(categoryTotal, 0), 2) AS categorySharePercent
      FROM ranked
      WHERE id = :id
    `, params);
    ...
    return {
      ...
      context: {
        ...
        spendSharePercent: row.spendSharePercent,
        differenceFromAverage: row.differenceFromAverage,
        filteredAverage: row.roundedFilteredAverage,
        categoryAverage: row.roundedCategoryAverage,
        categoryRank: row.categoryRank,
        categorySharePercent: row.categorySharePercent,
      },
    };
  });
}
